...
# COVARIANCES & SIGMA - SAME AS METHOD 1                                                        ####
...
##2. Sigma: covariance for 3 ensemble mean simulators on each date for one lead day
...
## 4. mean vector of the shared discrepancy on a date for one lead day.
error_PC1 <- PC1.pert.leadtime.ensemblemean_ALL - PC1_obs
error_PC2 <- PC2.pert.leadtime.ensemblemean_ALL - PC2_obs
error_TN  <- TN.pert.leadtime.ensemblemean_ALL - TN_obs
error_TS  <- TS.pert.leadtime.ensemblemean_ALL - TS_obs

eta <- array(0,dim=c(4,600,14))
lambda<- array(0,dim = c(4,4,600,14))
for (j in 1:14){
    for (i in 31:630){
        eta[,i-30,j] <- c(mean(error_TN[1:(i-1),j]),
                          mean(error_TS[1:(i-1),j]),
                          mean(error_PC1[1:(i-1),j]),
                          mean(error_PC2[1:(i-1),j]))
        lambda[,,i-30,j] <- cov(cbind(error_TN[1:(i-1),j],
                                      error_TS[1:(i-1),j],
                                      error_PC1[1:(i-1),j],
                                      error_PC1[1:(i-1),j]))
    }
