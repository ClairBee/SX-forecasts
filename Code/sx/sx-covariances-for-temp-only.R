...
# replication of Sichun's 'covariances for temp only' code, with additional files loaded where necessary
...
C_ECMWF <- array(0,dim=c(2,2,630,14))
for (j in 1:14){
    for (i in 1:630){
...
C_NCEP <- array(0,dim=c(2,2,630,14))
for (j in 1:14){
    for (i in 1:630){
...
C_UKMO <- array(0,dim=c(2,2,630,14))
for (j in 1:14){
    for (i in 1:630){
...
sigma<- array(0,dim=c(2,2,630,14))
for (j in 1:14){
    for (i in 1:630)
        sigma[,,i,j] <- cov(cbind(mean_TN[,i,j],mean_TS[,i,j]))
}
...
## 3. lambda: covariance of the shared discrepancy (7 years)
...
lambda<- array(0,dim = c(2,2,90,14))
for (j in 1:14){
    for (z in 1:90)
        lambda[,,z,j] <- cov(cbind(mean.error_TN_ALL[,z,j],mean.error_TS_ALL[,z,j]))
}
...
eta <- array(0,dim=c(2,90,14))
for (j in 1:14){
    for (i in 1:90){
        eta[,i,j] <- c(mean(mean.error_TN_ALL[,i,j]),mean(mean.error_TS_ALL[,i,j]))
    }
}
...
## 5. Y_bar on one date for one lead day
Y_bar_ECMWF <- array(0,dim=c(2,630,14))
for (j in 1:14){
    for (i in 1:630){
        Y_bar_ECMWF[,i,j] <- cbind(TN.pert.leadtime.ensemblemean_ECMWF[i,j],TS.pert.leadtime.ensemblemean_ECMWF[i,j])
    }
}

Y_bar_NCEP <- array(0,dim=c(2,630,14))
for (j in 1:14){
    for (i in 1:630){
        Y_bar_NCEP[,i,j] <- cbind(TN.pert.leadtime.ensemblemean_NCEP[i,j],TS.pert.leadtime.ensemblemean_NCEP[i,j])
    }
}

Y_bar_UKMO <- array(0,dim=c(2,630,14))
for (j in 1:14){
    for (i in 1:630){
        Y_bar_UKMO[,i,j] <- cbind(TN.pert.leadtime.ensemblemean_UKMO[i,j],TS.pert.leadtime.ensemblemean_UKMO[i,j])
    }
}
...
D_ECMWF <- array(0,dim = c(2,2,630,14))
D_NCEP <- array(0,dim = c(2,2,630,14))
D_UKMO <- array(0,dim = c(2,2,630,14))
precision <- array(0,dim = c(2,2,630,14))
S<- array(0,dim = c(2,2,630,14))
tau <- array(0,dim = c(2,630,14))
for (j in 1:14) {
    for (i in 1:630) {
        ...
        tau[,i,j] <- S[,,i,j]%*%(solve(diag(2)+solve(D_ECMWF[,,i,j])%*%lambda[,,(i-1)%%90+1,j]+solve(D_NCEP[,,i,j])%*%lambda[,,(i-1)%%90+1,j]+solve(D_UKMO[,,i,j])%*%lambda[,,(i-1)%%90+1,j])%*%(solve(D_ECMWF[,,i,j])%*%Y_bar_ECMWF[,i,j]+solve(D_NCEP[,,i,j])%*%Y_bar_NCEP[,i,j]+solve(D_UKMO[,,i,j])%*%Y_bar_UKMO[,i,j]))-eta[,(i-1)%%90+1,j]
    }
}
...
## RMSE
S_diag <- array(0,dim = c(2,630,14))
Spread <- array(0,dim=c(2,14))
for (j in 1:14) {
    for (i in 1:630) {
        for (k in 1:2){
...
observation <- cbind(TN_obs,TS_obs)
RMSE <- array(0,dim=c(2,14))
for (j in 1:14){
    for(k in 1:2) {
...
CRPS_Richard3 <- array(0,dim = c(2,630,14))
CRPS_Richard_average3 <- array(0,dim=c(2,14))

for (i in 1:630){
    for (j in 1:14){
        for (k in 1:2) {
            CRPS_Richard3[,i,j] <- verification::crps(observation[i,],cbind(tau[,i,j],sqrt(diag(S[,,i,j]))))$crps
            CRPS_Richard_average3[k,j] <- mean(CRPS_Richard3[k,,j])
        }
    }
}
